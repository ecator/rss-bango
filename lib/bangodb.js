...
class bangoDB{
	...
	//按照tag字段查找
	searchByTag(tag,table,callback){
		var that=this;
		this.connection.query("select * from "+table+" where tag like ? order by date desc",["%"+tag+"%"],function(err,res){
			if (err) {
				console.error(err);
			}else if(table=="detail" && !res.length){
				//没有在detail表中查询到，查询detail_mo表
				that.searchByTag(tag,"detail_mo",callback);
			}else{
				callback(res);
			}
		});
